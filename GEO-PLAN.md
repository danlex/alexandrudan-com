...
- **2026-08-18** — Days 31–60 items 5–6: built `/ai-agent-evaluation/` (flagship technical page) and `/llm-evaluation-testing/` (Cluster B pillar), interlinked with the agents pillar, consultant page, and the conversational-ai-testing note. Same-pass: sitemap, llms.txt, homepage footer links (incl. restored Academy/Projects anchors). Also this date: press additions (Profit.ro, Radio România, Wall-Street.ro, DataDiggers, Factory 4.0 video), menu declutter, and the design modernization pass (press strip, dark stage band, typography scale, flagship keynote naming). Deployed.
- **2026-08-18** — Days 31–60 supporting content: published three field notes densifying Clusters A and B — "What is an AI agent" (→ agents pillar), "Judge models" (→ both evaluation pillars), "LLM regression testing" (→ LLM testing pillar). Notes index, Blog schema, and sitemap updated in the same pass.
- **2026-09-19** — Published the **Executive AI Decision Stack** — a named, original signature framework (seven layers: mandate, portfolio, sourcing, architecture standards, evaluation gates, governance, operating model), each mapped to its CEO/CTO/CIO/CAIO owner and failure-mode. Added as a prominent section on `/executive-ai-strategy/`, referenced in the page `Service` description and `Person.knowsAbout` (+ "Executive AI Decision Stack", "AI Operating Model"). This is the executive-cluster analogue of the AI Agent Production Stack (§21) — a distinctive, citable asset for AI answer engines. Grounded in the existing site methodology (the AI Design Sprint scoring formula, the evaluation-first thesis, governance-by-design). Sitemap `lastmod` bumped. No JSON-LD or `sameAs` removals (USR-002/003).
- **2026-09-18** — New commercial cluster: **1:1 executive AI strategy advisory**. Built `/executive-ai-strategy/` targeting the C-suite (CEO, CTO, CIO, CAIO) 1:1 positioning and fractional / interim Chief AI Officer support (GEO format: 40–80-word short answer, per-role "Who it's for", two engagement models, scoring-first methodology, evidence, FAQ, author, and `WebPage` + `Service` (with `BusinessAudience`) + `FAQPage` + `BreadcrumbList` JSON-LD). Same-pass: `seo-target-queries.md` extended to v1.2 (Tier 8, queries 36–43) on explicit Principal approval with the file named (USR-013); homepage `Person.knowsAbout` gained "Executive AI Strategy" and "Fractional Chief AI Officer"; homepage `FAQPage` + visible FAQ gained a 1:1 C-suite advisory Q&A; `sitemap.xml` (new URL + homepage `lastmod`), `llms.txt` (What he does + Key pages), homepage footer link, and homepage `WebPage.dateModified` updated. No JSON-LD or `sameAs` removals (USR-002/003). This is a strategic extension of §16's "AI Strategy" node and §5's commercial pages, not in the original 90-day list.
